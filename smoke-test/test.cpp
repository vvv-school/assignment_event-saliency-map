...
//                                  "Could not connect yarpdataplayer to spiking model");
        ...
        ROBOTTESTINGFRAMEWORK_ASSERT_ERROR_IF_FALSE(yarp::os::Network::connect("/vSalMap/vBottle:o", "/spikechecker/vBottle:i", "udp"),
                                  "Could not connect saliency map to spike checker");

//        ROBOTTESTINGFRAMEWORK_TEST_REPORT("Ports successfully open and connected");

        spkchk.setROI(140, 60, 105, 80);
    ...
    virtual void run()
    {
        ...
        ROBOTTESTINGFRAMEWORK_ASSERT_ERROR_IF_FALSE(inliers > 7500, "Inlier score too low (7500)");
